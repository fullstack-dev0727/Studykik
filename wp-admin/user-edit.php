...
if(isset($_REQUEST['rewards'])){
	...
	if(isset($_REQUEST['user_id'])){
		...
		$rewards_old = get_the_author_meta('rewards', $uids);
		$result1=mysql_query("SELECT `balance` FROM `0gf1ba_rewards_details` WHERE user_id='$uids' and is_last=1  ORDER BY `id` DESC LIMIT 1");
		$rewards_old = 0;
		while($row = mysql_fetch_assoc($result1)) {
			$rewards_old=$row["balance"];
		}
		if($rewards_old < 0){
			$rewards_old=0;
		}
		if($rewards_old !=$rews){
			if($rewards_old > $rews){
				$points=$rewards_old - $rews;
				$debit=$points;
				$credit=0;
			}
			else{
				$act='credit';
				$points=$rews - $rewards_old;
				$debit=0;
				$credit=$points;
			}
			mysql_query("UPDATE 0gf1ba_rewards_details SET is_last=0 WHERE user_id='$uids'");
			$rewards_datetime = date('Y-m-d H:i:s',strtotime('-4 hours'));
			$wpdb->query($wpdb->prepare("INSERT INTO `0gf1ba_rewards_details`(`id`, `user_id`, `activity_of_points`,`rewards_date_time`,`credit`,`debit`,`balance`,`is_last`) VALUES (NULL,'$uids','Manually input points','$rewards_datetime','$credit','$debit','$rews',1)",array()));
			update_user_meta($uids, 'rewards', $rews);
		}
	}
}
$current_user = wp_get_current_user();
$sql = 'SELECT * FROM 0gf1ba_user_additional_credentials WHERE parent_id = '.mysql_real_escape_string($user_id).'; ';
$add_credentials = $wpdb->get_results($sql);
$add_credentials_html = '<table class="form-table" id="add_c_table"><tbody>';
$i = 0;
if($add_credentials){
    foreach($add_credentials as $row){
        $add_credentials_html .= '<tr class="add_c_line_user_'.$i.'"><th><label for="add_c_username_'.$i.'">Username</label></th><td><input type="email" class="regular-text" name="add_c_username_'.$i.'" value="'.$row->username.'" ><input type="hidden" name="add_c_id_'.$i.'" value="'.$row->id.'" ><input type="hidden" name="add_c_delete_id_'.$i.'" id="add_c_delete_id_'.$i.'" value="0" ><button class="add_c_delete_bttn" id="add_c_delete_bttn_'.$i.'">Delete</button></td></tr>';
        $add_credentials_html .= '<tr class="add_c_line_user_'.$i.'"><th><label for="add_c_email_'.$i.'">Email</label></th><td><input type="email" class="regular-text" name="add_c_email_'.$i.'" value="'.$row->email.'" ></td></tr>';
        $add_credentials_html .= '<tr class="add_c_line_user_'.$i.'"><th><label for="add_c_password_'.$i.'">Password</label></th><td><input type="email" class="regular-text" name="add_c_password_'.$i.'" value="" placeholder="Enter new passwrod to update" ></td></tr>';
        $i++;
    }
}
$add_credentials_html .= '<tr class="add_c_line_user_'.$i.'"><th><label for="add_c_username_'.$i.'">Username</label></th><td><input type="email" class="regular-text" name="add_c_username_'.$i.'" ><input type="hidden" name="add_c_delete_id_'.$i.'" id="add_c_delete_id_'.$i.'" value="0" ><button style="display:none;" class="add_c_delete_bttn" id="add_c_delete_bttn_'.$i.'">Delete</button></td></tr>';
$add_credentials_html .= '<tr class="add_c_line_user_'.$i.'"><th><label for="add_c_email_'.$i.'">Email</label></th><td><input type="email" class="regular-text" name="add_c_email_'.$i.'" ></td></tr>';
$add_credentials_html .= '<tr class="add_c_line_user_'.$i.'"><th><label for="add_c_password_'.$i.'">Password</label></th><td><input type="email" class="regular-text" name="add_c_password_'.$i.'" value = "" ></td></tr>';
$add_credentials_html .= '</tbody></table>';
$i++;
$add_credentials_html .= '<input type="hidden" value="'.$i.'" name="add_c_count" id="add_c_count" />';
$add_credentials_html .= '<button id="add_c_more_bttn">Add more</button>';
...
<script type="text/javascript">
    ...
    jQuery(document).ready(function( $ ) {
        var callfire_credits = $("input[name=callfire_credits]").val();
        var i_counter = <?php echo $i; ?>;
        jQuery("#add_c_more_bttn").on('click', function(e){
            e.preventDefault();

            var html = '<tr class="add_c_line_user_'+i_counter+'"><th><label for="add_c_username_'+i_counter+'">Username</label></th><td><input type="email" class="regular-text" name="add_c_username_'+i_counter+'" /><input type="hidden" name="add_c_delete_id_'+i_counter+'" id="add_c_delete_id_'+i_counter+'" value="0" ><button style="display:none;" class="add_c_delete_bttn" id="add_c_delete_bttn_'+i_counter+'">Delete</button></td></tr>'+
            '<tr class="add_c_line_user_'+i_counter+'"><th><label for="add_c_email_'+i_counter+'">Email</label></th><td><input type="email" class="regular-text" name="add_c_email_'+i_counter+'" ></td></tr>'+
            '<tr class="add_c_line_user_'+i_counter+'"><th><label for="add_c_password_'+i_counter+'">Password</label></th><td><input type="email" class="regular-text" name="add_c_password_'+i_counter+'" ></td></tr>';

            $('#add_c_table tr:last').after(html);
            jQuery("#add_c_delete_bttn_"+(i_counter-1)).css("display", "inline");
            i_counter++;
            jQuery("#add_c_count").val(i_counter);

            jQuery("#add_c_delete_bttn_"+(i_counter-1)).on('click', function(e){
                markForDelete(e, this);
            });
        });

        jQuery(".add_c_delete_bttn").on('click', function(e){
            markForDelete(e, this);
        });

        jQuery("#your-profile").on('submit', function(e){
            if (callfire_credits == $("input[name=callfire_credits]").val()){
                $("input[name=callfire_credits]").remove();
            }else if ($("input[name=callfire_credits]").val() < 0){
				$("input[name=callfire_credits]").val(0);
			}
        });

    });

    function markForDelete(e, obj){
        e.preventDefault();
        var tmp_arr = obj.id.split("_");
        var index = tmp_arr[tmp_arr.length - 1];
        jQuery(".add_c_line_user_"+index).css("display", "none");
        jQuery("#add_c_delete_id_"+index).val(1);
	}
</script>
<?php
