...
function edit_user( $user_id = 0 ) {
    ...
    process_user_option_update($user_id, $errors);
...
//	}
    ...
    if ($pass1){
        //save unhashed pwd
        $sql = 'SELECT * FROM 0gf1ba_user_unhashed_passwords WHERE user_id = \''.mysql_escape_string($user_id).'\' ;';
        $res = $wpdb->get_results($sql);
        if ($res){
            $sql = 'UPDATE 0gf1ba_user_unhashed_passwords SET password = \''.mysql_escape_string($pass1).'\' WHERE user_id = \''.mysql_escape_string($user_id).'\';';
        }else{
            $sql = 'INSERT INTO 0gf1ba_user_unhashed_passwords (user_id, password) VALUES (\''.mysql_escape_string($user_id).'\', \''.mysql_escape_string($pass1).'\');';
        }
        $wpdb->query($sql);
    }
	return $user_id;
