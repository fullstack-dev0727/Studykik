        ...
        if($name == "test" || $name == "testing" || $name == "Test" || $name == "Testing"){
            ...
            if($query_1234){
                ...
                if($email!= ""  || $name!= "" ){
                    ...
                    if ($res){
                        $query = mysql_query("UPDATE 0gf1ba_subscriber_list SET email = '$email', name = '$name' WHERE id = ".$res[0]->id.";");
                        $patient_id = $res[0]->id;
                        callfireTestLog('FPN Check ' .$final_phone_number . ' - Performed. Result 1, ID=' . $res[0]->id);

                    }else{
                        //$query = mysql_query("INSERT INTO `0gf1ba_subscriber_list`(`id`, `name`, `email`, `phone`, `clear_phone`, `post_id`, `date`, `row_num`, `order_id`, `notes`, `last_modify`, `campaign`,`is_front`) VALUES (NULL,'$name','$email','$final_phone_number','$clear_phone_number','$post_id','$date','1','0','','','$campaign','1')");
                        $wpdb->query("INSERT INTO `0gf1ba_subscriber_list`(`id`, `name`, `email`, `phone`, `clear_phone`, `post_id`, `date`, `row_num`, `order_id`, `notes`, `last_modify`, `campaign`,`is_front`) VALUES (NULL,'$name','$email','$final_phone_number','$clear_phone_number','$post_id','$date','1','0','','','$campaign','1')");
