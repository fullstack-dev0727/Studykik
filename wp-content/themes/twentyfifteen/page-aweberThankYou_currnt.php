<?php
/**
Template Name: Thank You Aweber Currnt
 */
get_header(); ?>
<style>
#inner-page {
...
$final_phone_number = $phone_number12.''.$phone_number123;
	 ...
	 <?php }else{?>
      ...
      <div class="ssba" style="margin:0 0 -40px 0">
        <div style="text-align:left"><span>Share this</span><br>
          <a target="_blank" href="http://www.facebook.com/sharer.php?u=<?php the_permalink(); ?>" class="ssba_facebook_share"><img alt="Share on Facebook" class="ssba" title="Facebook" src="http://studykik.com/wp-content/plugins/simple-share-buttons-adder/buttons/somacro/facebook.png"></a>

          <a target="_blank" href="http://twitter.com/share?url=<?php the_permalink(); ?>&amp;text=<?php the_title();?>" class="ssba_twitter_share"><img alt="Tweet about this on Twitter" class="ssba" title="Twitter" src="http://studykik.com/wp-content/plugins/simple-share-buttons-adder/buttons/somacro/twitter.png"></a>

          <a target="_blank" href="http://www.tumblr.com/share/link?url=studykik.com/constipation-omnispec-2834464/&amp;name=<?php the_title();?>"><img alt="Share on Tumblr" class="ssba" title="tumblr" src="http://studykik.com/wp-content/plugins/simple-share-buttons-adder/buttons/somacro/tumblr.png"></a>

          <a href="mailto:?Subject=<?php the_title();?>&amp;Body=%20<?php the_permalink(); ?>" class="ssba_email_share"><img alt="Email this to someone" class="ssba" title="Email" src="http://studykik.com/wp-content/plugins/simple-share-buttons-adder/buttons/somacro/email.png"></a></div>
      </div>
      <br/>
...
$callfire_contact=$_REQUEST['custom_mobile_phone_number'];
...
foreach($questions_arr as $in=> $qes){
	...
	$variable.='<if expr="${incorrect} ==1" name="checkWrong">';
            ...
            //$variable.='<transfer callerid="$'.$from_number.'" name="transferwrong">'.$number.'</transfer>';
              $variable.='<transfer callerid="${call.phonenumber}" name="transferwrong">'.$number.'</transfer>';
              }
              else
              {
            $variable.='<transfer name="transferwrong">'.$number.'</transfer>';
              }
	$variable.='</if>';
        $variable.='<if expr="${iscorrect} ==2" name="checkRight">';
            ...
            if($from_number == $number){
            $variable.='<transfer callerid="${call.phonenumber}" name="transferright">'.$number.'</transfer>';
            }
          else {
           $variable.='<transfer name="transferright">'.$number.'</transfer>';
              }
...
$request->ToNumber = array($_REQUEST['custom_mobile_phone_number']); // required choice
    ...
    if($number!='' && $number!='0' && $from_number!='' && $from_number!='0' && empty($questions_arr)){
        ...
        $request->Broadcast->IvrBroadcastConfig->RetryConfig->RetryResults = 'AM BUSY NO_ANS';
         ...
         $variable.='<play type="callfireid">1715427003</play>';
           if($from_number == $number){
           $variable.='<transfer callerid="${call.phonenumber}" name="transferdefault">'.$number.'</transfer></dialplan>';
            }
            else{
               $variable.='<transfer name="transferdefault">'.$number.'</transfer></dialplan>';
            }
