                                        ...
                                        if ($post_author_id) {
                                            if ($post_author_id > 0) {
                                                ...
                                                if ($user->user_login != "") {
													...
													?>
							  <?php if ($user_roles == "manager_username"){?>
							  <div style="width:69.5% !important;float:left;">&nbsp;</div>
...
if(isset($_REQUEST['aid'])){
        ...
        <h4 id="linke">  <a href="<?php echo site_url();?>/invoice-receipts/" style="color:#f78f1e">  INVOICE RECEIPTS </a></h4></div>
...
<a href="<?php echo site_url();?>/invoice-receipts/?d=download&aid=<?php echo $sel_val;?>" class="download_invoice" style="margin-left:160px;"><img class="img-responsive" alt="" src="<?php bloginfo('template_url');?>/images-dashboard/downlode_btn.png"></a>
</form>
		...
		$query_invoice_number = $wpdb->get_results( "SELECT * FROM `0gf1ba_invoice_number` WHERE user_id in ($all_ids) and month = '$month' and year = '$year' ORDER BY `id` DESC");
		...
		$query_invoice_number = $wpdb->get_results( "SELECT * FROM `0gf1ba_invoice_number` WHERE user_id in ($all_ids) ORDER BY `id` DESC");
        ...
        <td class="second_row"><?php echo "$".number_format( str_replace("$", "", str_replace(",", "", $query_invoice_number_value->price)) ,  2 ,  '.' ,  ',' );?></td>
		...
		<?php   }
         ...
         <tr>
        <td colspan="4" class="second_row" style="border-right:none !important;"> No Invoice found!</td>
