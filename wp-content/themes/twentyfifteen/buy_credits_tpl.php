...
.outer_p, .buy_credits_radio_container{
    padding-left:15px;
}

.buy_credits_container p, .buy_credits_radio_container{
    width: 100%;
    font: 18px Arial, Helvetica, sans-serif;
    font-weight: bold;
}

.buy_credits_container label{
    width: 100%;
    float: left;
    font: 18px Arial, Helvetica, sans-serif;
    font-weight: bold;
...
.buy_credits_container p select{
    float: left;
    font: 18px Arial, Helvetica, sans-serif;
    height: 46px;
    margin: 0 0 13px;
...
.buy_credits_controls_container{
    width:100%;
}

.buy_credits_purchase_bttn{
...
.buy_credits_price_table{
    width:100%;
    font-size: 15px;
    font-weight: bold;
}
.buy_credits_price_table td{
    text-align: left;
    vertical-align: baseline;
}

.buy_credits_price_table input,
.buy_credits_price_table textarea,
.buy_credits_price_table select{
    font: 18px Arial, Helvetica, sans-serif;
    height: 46px;
    margin: 0 0 13px;
    padding: 0 2%;
...
#loop_credit .Add_cart {
    padding-bottom: 20px;
}
#loop_credit .add_new_cart {
    padding: 0;
}
#loop_credit .add_new_cart h2 {
...
function changePriceValue(){
    $("#credits").val((100 * $("#quantity").val()));
    $("#price").val("$"+(77 * $("#quantity").val()));
    jQuery(".buy_credits_error_text").html("You have successfully purchased "+(100 * $("#quantity").val())+" credits.");
}

$(document).ready(function () {

    $('.go-add-card-credit').click(function(){

        var ajaxurl     = '/wp-admin/admin-ajax.php';
        var datastring  = jQuery(".add-card-form-credit").serialize();
        var errors = 0;
        var data_form = $(this).closest('form');
        jQuery('.add-card-status-credit').html("");
        ...
        if (errors == 0) {
            ...
            if(/^[a-zA-Z0-9-_ ]*$/.test(str) == false) {
                ...
                data_form.find('.add-card-status-credit').html("Company field can't contain symbols.");
                data_form.find('.add-card-status-credit').css('color','#ff0000');
                errors ++;
            } else {
                data_form.find("input[name=company]").removeClass("warning");
                data_form.find('.add-card-status-credit').html("");
            }
        }
        ...
        if (someday < today && errors == 0) {
            ...
            data_form.find('.add-card-status-credit').html("The expiration date is invalid.");
            data_form.find('.add-card-status-credit').css('color','#ff0000');
            return false;
        }

        if (errors == 0) {
            $('.add-card-status-credit').html('Processing...');
            $.ajax({
                type: "POST",
                url: ajaxurl,
                data: datastring,
                success: function(data) {
                    ...
                    if (!json.error){
                        $('.select-card-credit').append('<option value="'+json[0].card_id+'" data-shipping-id="'+json[0].shipping_id+'" data-credit-card-id="'+json[0].card_id+'" data-profile-id="'+json[0].profile_id+'" data-cvv="'+json[0].cvv+'" data-payment-id="'+json[0].payment_id+'">xxxx xxxx xxxx '+json[0].last_4+'</option>');
                        console.log(json);
                        ...
                        jQuery('.add-card-status-credit').html(json.messages[0]);
                        jQuery('.add-card-status-credit').css('color','#ff0000');
...
//                        jQuery('input[name=cc_cvv2]').addClass("warning");
                ...
                error: function(data){
                    console.log(data);
                    jQuery('.add-card-status-credit').html(data);
                    jQuery('input[name=cc_number]').addClass("warning");
                    jQuery('select[name=cc_exp_month]').addClass("warning");
                    jQuery('select[name=cc_exp_year]').addClass("warning");
//                    jQuery('input[name=cc_cvv2]').addClass("warning");
    ...
    jQuery("#card_cancel_credit").on('click',function(){
        jQuery("#loop_credit").hide();
        var data_form = $('.add-card-form-credit');
        data_form.find("input").map(function () {
            $(this).val("");
            $(this).removeClass('warning');
        });
    ...
    jQuery(".select-card-credit").on('change',function(){

         var sel_val=jQuery(this).val();
         if(sel_val=='Add a Card'){
            jQuery("#loop_credit").show();
             $("#loop_credit input[type=text]").map(function(){
                 $(this).val("");
             });
             $("#loop_credit select").map(function(){
                 $(this).val("");
             });
         }
         else{
    ...
    $("#buy_credits_purchase_bttn").on('click', function(){
        var errors = 0;
        $("#buy_credits_form .required").map(function(){
...
//                    $("#fullname").focus();
        ...
        $("#buy_credits_purchase_bttn").prop('disabled', true);
        jQuery.ajax({
            async: true,
            url: "<?=get_bloginfo('wpurl')?>/wp-admin/admin-ajax.php",
            type:'POST',
            data: "action=buy_credits&post_id="+ $("#buy_credits_post_id").val()+"&"+$( "#buy_credits_form" ).serialize(),
            success: function(data){
                data = data.substr(0, (data.length-1));
                data = JSON.parse(data);
                if (data.success){
                    jQuery(".buy_credits_form_area").css("height","80px");
                    jQuery("#buy_credits_title").html("Thank you");
                    jQuery("#buy_credits_title").css("text-decoration", "none");
                    jQuery("#buy_credits_close_top_bar").css("display", "none");
                    jQuery("#buy_credits_close_bttn").show();
                    jQuery("#buy_credits_purchase_bttn").hide();
                    jQuery(".buy_credits_error_text").show();
                    jQuery(".buy_credits_main_content").hide();

                }else{
                    alert("Error. Please try again later.");
                }
                $("#buy_credits_purchase_bttn").prop('disabled', false);
            }
        });
    });

    $("#buy_credits_close_bttn").on('click', function(){
        $(".buy_credits_wrapper_container a").click();
    });
    ...
    <div id="buyCredits" class="white_content" style="width:cursor: auto; display: none;">
        <input type="hidden" id="buy_credits_post_id" />
        <div class="col-xs-12 col-md-12 notes_left buy_credits_wrapper_container">
            <div class="row">
                <h2 id="buy_credits_title">'.$post_id.'</h2>
            </div>
            <div data-offset="0" data-target="#myNavbar" data-spy="scroll" class="scroll-area_connect buy_credits_form_area buy_credits_container" id="nts_diva" style="height:320px;">
                <form id="buy_credits_form">
                    <div class="buy_credits_error_text" >You have successfully purchased 100 credits.</div>
                    <div class="buy_credits_main_content">
                        <p class="outer_p" style="margin-top:10px;">
                            <span class="wpcf7-form-control-wrap">
                                <div class="buy_credits_price_container">
                                    <table class="buy_credits_price_table">
                                        <tr><td width="150">Quantity</td><td><input id="quantity" type="number" min="1" value="1" name="quantity" onkeypress="return isNumber(event)" onkeyup="changePriceValue()"></td></tr>
                                        <tr><td>Credits</td><td><input id="credits" type="text" value="100" name="credits" readonly="true"></td></tr>
                                        <tr><td>Price</td><td><input id="price" type="text" value="$77" name="price" readonly="true"></td></tr>
                                        <?php
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <select name='select_card' class='select-card-credit required'>
...
<!--                                        <tr>-->
<!--                                            <td colspan='3'>-->
<!--                                                <textarea value="" placeholder="Notes" id="select-card-credit" name="notes" class="select-card-credit" rows="3"/></textarea>-->
<!--                                            </td>-->
<!--                                        </tr>-->
            ...
            <div class="buy_credits_controls_container">

                <input type="button" class="center-block buy_credits_purchase_bttn" value="Purchase" id="buy_credits_purchase_bttn">
                <input type="button" class="center-block close_button" value="Close" id="buy_credits_close_bttn" style="display:none; margin-bottom:15px;">
            </div>
        </div>

    </div>

    <div id="loop_credit" class="add_payment_popup_credit">
        <form method="post" action="#" class="add-card-form-credit">
            <div class="Add_cart">
...
<!--                </div>-->
                <div class="col-sm-6 col-xs-12 add_cancel_btn">
                    <a href="#"><img id="card_cancel_credit" src="<?= get_bloginfo('template_url')?>/images/cancel.png" alt="" class="img-responsivem pull-right"></a>
                </div>
                <div class="col-sm-6 col-xs-12 add_save_btn">
                    <a href="#" class="go-add-card-credit"><img src="<?= get_bloginfo('template_url')?>/images/save.png" alt="" class="img-responsive pull-left"></a>
                    <input type="hidden" name="action" value="addcard" />
                    <input type="hidden" name="post_id" class="post_id" value="" />
                    <input type="hidden" name="user_id" value="<?php echo get_current_user_id();?>" />
                </div>
                <div style="clear:both; font-weight: bold; padding: 20px 0; text-align: center;" class="add-card-status-credit"></div>
            </div>
        </form>
    </div>