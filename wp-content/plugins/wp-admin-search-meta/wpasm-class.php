...
class WPASM_Search {
	...
	public static function posts_where( $where ) {
		...
		$where = preg_replace( "/($wpdb->posts.post_title LIKE '%{$wp->query_vars['s']}%')/i", "$0 OR ( ($wpdb->postmeta.meta_key = 'study_no') AND ($wpdb->postmeta.meta_value LIKE '%{$wp->query_vars['s']}%')) ", $where );
		
		//$where .= " OR ( ($wpdb->postmeta.meta_key = '') AND  ($wpdb->postmeta.meta_value = '{$wp->query_vars['s']}') ) ";
