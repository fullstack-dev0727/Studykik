	...
	if ( isset( $gmw['post_types'] ) && count( $gmw['post_types'] ) > 1 ) :
		...
		echo '<select name="wppl_post" id="gmw-posts-dropdown-'.$gmw['form_id'].'" class="gmw-posts-dropdown gmw-posts-dropdown-'.$gmw['form_id'].' '.$class.'">';
			...
			foreach ($gmw['post_types'] as $post_type) :
				...
				if ( $_POST["wppl_post"] == $post_type ) $pti_post = 'selected="selected"'; else $pti_post = "";
		...
		foreach ($gmw['taxonomies'] as $tax => $values) :
			...
			if ( isset( $values['style']) && $values['style'] == 'drop') :
				...
				$output = '<div id="'.$tax . '-tax-wrapper" class="gmw-single-taxonomy-wrapper gmw-dropdown-taxonomy-wrapper gmw-dropdown-'.$tax. '-wrapper">';
					...
					if (isset($_POST['tax_'.$tax])) $get_tax = $_POST['tax_'.$tax];

                    if(is_home() || is_page(51)|| is_page(4)){
					...
					$args = array(
						...
						'class'				=> 'gmw-dropdown-'.$tax.' gmw-dropdown-taxonomy styled',
						...
						'show_option_all'   => __('Any Type','GMW'),
						...
						'taxonomy' 			=> 'featured-research-center',
	...
	if ( count($perPage) > 1 ) :
		...
		$per_page = '<select name="gmw_per_page" id="gmw-pt-per-page-dropdown-'.$gmw['form_id'].'" class="gmw-pt-per-page-dropdown '.$class.'">';
			...
			foreach ( $perPage as $pp ) :
				...
				if ( isset($_POST['wppl_per_page']) && $_POST['wppl_per_page'] == $pp ) $pp_s = 'selected="selected"'; else $pp_s = "";
...
function gmw_pt_query_taxonomies($tax_args, $gmw) {
	...
	$ptc = ( isset($_POST['wppl_post'] ) ) ? count( explode( " ",$_POST['wppl_post']) ) : count($gmw['post_types']);
...
function gmw_pt_within( $gmw,$sm, $om, $rm ,$wm, $fm, $nm ) {
	...
	if ( isset( $_POST['action'] ) && $_POST['action'] == 'wppl_post' ) $nm = $gmw['org_address'];
...
function gmw_pt_directions($gmw, $post, $title) {
    ...
    echo 	'<span><a href="https://maps.google.com/maps?f=d&hl='.$region[0].'&region='.$region[1].'&doflg=' . $gmw['units_array']['map_units'] . '&geocode=&saddr=' . $gmw['org_address'] . '&daddr=' . str_replace(" ", "+", $post->address) .'&ie=UTF8&z=12" target="_blank">'.$title.'</a></span>';
...
function gmw_pt_no_results($gmw, $gmw_options) {
	...
	$no_results = '<div class="gmw-pt-no-results-wrapper"><h3>No Studies Currently Listed for that Type</h3></div>';
...
function gmw_get_posts_results($gmw, $gmw_options) {
	...
	$gmw['get_per_page'] = ( isset($_POST['wppl_per_page']) ) ? $_POST['wppl_per_page'] : current(explode(",", $gmw['per_page']));

	$gmw_post = ( isset( $_POST['wppl_post'] ) && !empty( $_POST['wppl_post']) )  ? explode(' ' , $_POST['wppl_post']) : $gmw['post_types'];
