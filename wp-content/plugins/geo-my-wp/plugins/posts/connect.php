...
if ( is_admin() ) {
	...
	function gmw_register_admin_location_scripts() {
		...
		if( isset( $gmw_options['address_fields'] ) && !empty( $gmw_options['address_fields'] ) && ( in_array( $post_type, $gmw_options['address_fields'] ) ) ) {
			...
			wp_register_script( 'google-api-key', 'https://maps.googleapis.com/maps/api/js?key='.$gmw_options['google_api'].'&sensor=false&region='.$gmw_options['country_code'],array(),false);
