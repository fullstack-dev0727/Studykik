...
?>
	...
	<div id="gmw-submit-wrapper-<?php echo $gmw['form_id']; ?>" class="gmw-submit-wrapper">
		...
		<input type="hidden" id="prev-address-<?php echo $gmw['form_id']; ?>" class="prev-address" value="<?php if ( isset($_POST['wppl_address']) ) echo implode(' ' ,$_POST['wppl_address']); ?>">

		<input type="hidden" id="gmw-lat-<?php echo $gmw['form_id']; ?>" class="gmw-lat" name="wppl_lat" value="<?php if ( isset($_POST['wppl_lat']) ) echo $_POST['wppl_lat']; ?>">

		<input type="hidden" id="gmw-long-<?php echo $gmw['form_id']; ?>" class="gmw-long" name="wppl_long" value="<?php if ( isset($_POST['wppl_long']) ) echo $_POST['wppl_long']; ?>">
		...
		<input type="submit" id="gmw-submit-<?php echo $gmw['form_id']; ?>" class="gmw-submit btn" name="find_search" value="<?php _e($subValue,'GMW'); ?>" /><div class="loading_image" style="display:none"><img src="<?php bloginfo('template_url');?>/images/ajax-loader.gif" /></div>
...
function gmw_search_form_address_field($gmw, $class) {
	...
	$address_field .=  '<input type="text" name="wppl_address[]" id="gmw-address" class="'.$am.' gmw-address gmw-full-address gmw-address-'.$gmw['form_id'].' '.$class.'" value="Zip Code" size="35" '; if ( isset( $gmw['address_title_within'] ) )  $address_field .=  ' />';
...
function gmw_search_form_radius_values($gmw, $class, $btitle, $stitle) {
	...
	if ( empty( $stitle ) ) $stitle = ( $gmw['units_name'] == 'imperial' ) ? __('Distance ','GMW') : __('- Kilometers -','GMW');
	...
	if ( count( $miles) > 1 ) :
		...
		echo '<span class="drop-cus"><select name="wppl_distance" class="gmw-distance-select styled gmw-distance-select-1 " id="gmw-distance-select-1"><option value="8000">Distance </option><option value="10">10 Miles</option><option value="50">50 Miles</option><option value="100">100 Miles</option><option value="250">250 Miles</option></select>';
...
function gmw_search_form_units($gmw, $class ) {
	...
	if ( $gmw['units_name'] == 'both' ) :
		...
		if ( isset($_POST['wppl_units']) && $_POST['wppl_units'] == 'metric')  $unit_m = 'selected="selected"'; else $unit_i = 'selected="selected"';
...
function gmw_shortcode($params) {
	...
	if ( isset( $_POST['action'] ) && $_POST['action'] == "wppl_post" && $gmw['form_id'] == $_POST['wppl_form'] && !isset( $params['widget']) && $gmw['results_page'] == false ) :
...
function gmw_form_submitted($gmw, $gmw_options) {
	...
	if ( isset($_POST['wppl_units']) && $_POST['wppl_units'] == "imperial" )
	...
	$gmw['radius'] = $_POST['wppl_distance'];
	...
	if ( array_filter($_POST['wppl_address']) ) $gmw['org_address'] = str_replace('+', ' ', implode( ' ', $_POST['wppl_address'] ));
	...
	if ( isset($gmw['org_address']) && !empty($gmw['org_address'])) :
		...
		$gmw['your_lat']  = $_POST['wppl_lat'];

		$gmw['your_long'] = $_POST['wppl_long'];
...
function gmw_results_shortcode() {
	...
	if ( !empty( $_POST['action'] ) &&  $_POST['action'] == "wppl_post" ) :

	

		$options_r = get_option('wppl_shortcode');

		$gmw = $options_r[$_POST['wppl_form']];
