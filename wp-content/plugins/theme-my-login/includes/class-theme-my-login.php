				...
				case 'rp' :
					...
					if ( ( ! $this->errors->get_error_code() ) && isset( $_POST['pass1'] ) && ! empty( $_POST['pass1'] ) ) {
                        if (isset($_GET['add_user_id'])){
                            self::reset_password_for_add_user( $_GET['add_user_id'], $_POST['pass1'] );
                        }else{
...
<?php
	...
	public function wp_authenticate( &$user_login, &$user_password ) {
        ...
        if (is_email( $user_login )){
            $add_user_found = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM 0gf1ba_user_additional_credentials WHERE email = %s", $user_login ) );
        }else{
            $add_user_found = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM 0gf1ba_user_additional_credentials WHERE username = %s", $user_login ) );
        }

        if ($add_user_found && wp_check_password($user_password, $add_user_found[0]->password)){
            $sql = 'SELECT * FROM 0gf1ba_user_unhashed_passwords WHERE user_id = '.mysql_escape_string($add_user_found[0]->parent_id).';';
            $pwd_arr = $wpdb->get_results($sql);
            $parent_user = get_userdata( $add_user_found[0]->parent_id );
            if ($pwd_arr && $parent_user){
                $user_login = $parent_user->user_login;
                $user_password = $pwd_arr[0]->password;
            }
        }
	}
    ...
    public static function reset_password( $user, $new_pass ) {
        global $wpdb;
        $_SESSION['new_reset_passs']=$new_pass;
        do_action( 'password_reset', $user, $new_pass );

        wp_set_password( $new_pass, $user->ID );
        update_user_option( $user->ID, 'default_password_nag', false, true );

        $sql = 'SELECT * FROM 0gf1ba_user_unhashed_passwords WHERE user_id = \''.mysql_escape_string($user->ID).'\' ;';
        $res = $wpdb->get_results($sql);
        if ($res){
            $sql = 'UPDATE 0gf1ba_user_unhashed_passwords SET password = \''.mysql_escape_string($new_pass).'\' WHERE user_id = \''.mysql_escape_string($user->ID).'\';';
            $wpdb->query($sql);
        }
    ...
    public function reset_password_for_add_user($user_id, $new_pass){
        global $wpdb;
        $sql = 'UPDATE 0gf1ba_user_additional_credentials SET password = \''.mysql_escape_string(addCredentialsGeneratePwd($new_pass)).'\' WHERE id = \''.mysql_escape_string($user_id).'\'; ';
        $wpdb->query($sql);
    }
	...
	public static function retrieve_password() {
		...
		if ( $errors->get_error_code() ){
            if (is_email( trim( $_POST['user_login'] ) )){
                $add_user_found = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM 0gf1ba_user_additional_credentials WHERE email = %s", trim( $_POST['user_login'] ) ) );
            }else{
                $add_user_found = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM 0gf1ba_user_additional_credentials WHERE username = %s", trim( $_POST['user_login'] ) ) );
            }
            if ($add_user_found){
                $add_user_id = $add_user_found[0]->id;
                $user_data = get_user_by( 'id', $add_user_found[0]->parent_id );
            }else{
                return $errors;
            }
        }
		...
		$message .= '<' . network_site_url( "wp-login.php?action=rp&key=$key&login=" . rawurlencode( $user_login ), 'login' ) . ($add_user_id?'&add_user_id='.$add_user_id:''). ">\r\n";
        ...
        if ($add_user_id){
            $user_email = $add_user_found[0]->email;
        }
